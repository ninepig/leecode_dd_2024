class Solution:
    ## easy way , sort ,take first k
    def findClosestElements(self, arr: List[int], k: int, x: int) -> List[int]:
        arr.sort(key=lambda v: abs(v - 4))
        return arr[0:k]
...
'''
    def findClosestElements(self, arr: List[int], k: int, x: int) -> List[int]:
        ...
        left = 0
        right = size - 1 - k
        while left < right : # approching way ,因为是连续的子数组 增序, 所以找差值最接近的 找两侧的极限值
            mid = left + (right - left) //2
            if x - arr[mid] > arr[mid + k] - x: # 左侧较大,所以目标值在右侧
                left = mid + 1
            else:
                right = mid
