...
# expect timestamp in form h:mmPM or hh:mmAM.
# def getMinutesFromTimestamp(ts):
#     colon = ts.index(':')
#
#     hour = int(ts[:colon])
#     minute = int(ts[colon + 1:colon + 3])
#     pm = ts[colon + 3:].lower() == "pm"
#
#     ret = 0
#     if hour == 12:
#         hour = 0
...
#
#
# def minuteDifference(start, end):
#     WHOLE_DAY = 1440
#
#     if start <= end:
#         return end - start
#     else:
#         return WHOLE_DAY - start + end
#
#
# def getActiveTime(activity):
#     earliest = None
#     pickups = 0
#
#     activeTimeTotal = 0
#     for a in activity:
#         ts, actionType = a.split('|')
#
#         minutes = getMinutesFromTimestamp(ts.strip())
#         isPickup = actionType.strip().lower() == "pickup"
#
#         if isPickup:
#             pickups += 1
#
...
#
#             if pickups == 0:
#                 activeTimeTotal += minuteDifference(earliest, minutes)
#
